...
namespace mega_gdn_native_decode_pto {
...
// existing AIV-only software barrier. mega_gdn_native_decode does not call it.
...
// Metadata is read on device on every replay. Slot zero and invalid rows
// are no-ops; all checkpoint IDs are checked before any persistent write.
AICORE PTO_INLINE bool NativeRowIsLive(
    __gm__ int* ids, __gm__ int* starts, __gm__ int* accepted,
    int32_t row, int32_t width, int32_t tokens, int32_t slots,
    int32_t conv_rows) {
  const int32_t start = starts[row];
  const int32_t length = starts[row + 1] - start;
  const int32_t a = accepted[row];
  if (start < 0 || length <= 0 || length > width ||
      start > tokens - length || a < 1 || a > width ||
      a + 2 > conv_rows || length + 2 > conv_rows) return false;
  const int32_t selected = ids[row * width + a - 1];
  if (selected <= 0 || selected >= slots) return false;
  for (int32_t i = 0; i < length; ++i) {
    const int32_t slot = ids[row * width + i];
...
AICORE PTO_INLINE void RunConvPhase(
    ...
    int32_t total_tokens, int32_t state_slots, int32_t conv_rows,
    int32_t qkv_stride,
  ...
  for (int32_t conv_tile = vector_core_idx; conv_tile < conv_tile_count;
    ...
    for (int32_t batch_idx = 0; batch_idx < batch_size; ++batch_idx) {
      if (!NativeRowIsLive(read_state_indices_handle, write_state_indices_handle,
                           num_accepted_tokens_handle, batch_idx, sequence_length,
                           total_tokens, state_slots, conv_rows)) continue;
      const int32_t start = write_state_indices_handle[batch_idx];
      const int32_t length = write_state_indices_handle[batch_idx + 1] - start;
      const int32_t read_slot = read_state_indices_handle[batch_idx * sequence_length];
      const int32_t write_slot = read_slot;
      const int32_t accepted =
          *(num_accepted_tokens_handle + batch_idx);
      const int64_t read_history_offset =
          static_cast<int64_t>(read_slot) * conv_state_stride +
      ...
      StoreBf16Row(
          conv_state_out_handle +
              static_cast<int64_t>(write_slot) * conv_state_stride + channel_offset,
          kUbConvHistoryHalf1);
      StoreBf16Row(
          conv_state_out_handle +
              static_cast<int64_t>(write_slot) * conv_state_stride + conv_dim + channel_offset,
          kUbConvHistoryHalf2);
      set_flag(PIPE_MTE3, PIPE_MTE2, EVENT_ID0);
      wait_flag(PIPE_MTE3, PIPE_MTE2, EVENT_ID0);

      for (int32_t token_idx = 0; token_idx < length;
           ++token_idx) {
        const int32_t token_offset =
            (start + token_idx) * conv_dim +
            channel_offset;
        ...
        if constexpr (SpeculativeTokens == 8) {
          ...
          LoadBf16Row(
              qkv_handle + static_cast<int64_t>(start + token_idx) * qkv_stride + channel_offset, conv_input_half_address);
          ...
          LoadBf16Row(qkv_handle + static_cast<int64_t>(start + token_idx) * qkv_stride + channel_offset, kUbConvInputHalf);
        ...
        if constexpr (SpeculativeTokens == 8) {
          ...
          StoreBf16Row(
              conv_state_out_handle +
                  static_cast<int64_t>(write_slot) * conv_state_stride +
...
AICORE PTO_INLINE void RunRecurrentStep(
  ...
  mega_gdn_decode_pto::VectorBarrier();
  // vLLM fused_gdn_gating keeps g in FP32; only beta is BF16.
  // The legacy SDK MTP oracle rounded g, which is not the model contract.
  TEXP(scalar_work, scalar_tmp);
  set_flag(PIPE_V, PIPE_S, EVENT_ID0);
  wait_flag(PIPE_V, PIPE_S, EVENT_ID0);
...
AICORE PTO_INLINE void Run(
    ...
    int32_t native_conv_stride, int32_t native_ssm_stride,
    int32_t total_tokens, int32_t state_slots, int32_t conv_rows,
    int32_t qkv_stride, int32_t z_stride) {
  ...
  const int32_t conv_state_stride = native_conv_stride;
  const int32_t v_heads_per_k = num_v_heads / num_k_heads;
  const int32_t v_width = num_v_heads * kHeadDim;
  const int32_t ssm_checkpoint_stride = native_ssm_stride;
  ...
  RunConvPhase<SpeculativeTokens>(
      ...
      conv_tile_count,
      conv_state_stride,
      total_tokens, state_slots, conv_rows, qkv_stride,
      vector_core_idx,
      vector_core_count);
#endif
  ...
  for (int32_t head_index = vector_core_idx;
    ...
    const int32_t start = write_state_indices_handle[batch_idx];
    const int32_t length = write_state_indices_handle[batch_idx + 1] - start;
    const int32_t accepted = num_accepted_tokens_handle[batch_idx];
    const int32_t read_slot = read_state_indices_handle[
        batch_idx * sequence_length + accepted - 1];
    ...
    for (int32_t token_idx = 0; token_idx < length;
      ...
      if constexpr (SpeculativeTokens == 8) {
        ...
        LoadBf16Row(
            ...
            kUbVHalf);
        const int32_t scalar_offset =
            (start + token_idx) * num_v_heads +
            head_idx;
        LoadBf16Scalar(a_handle + scalar_offset, kUbAHalf);
        LoadBf16Scalar(b_handle + scalar_offset, kUbBHalf);
        LoadBf16Row(
            z_handle +
                static_cast<int64_t>(start + token_idx) * z_stride +
                head_idx * kHeadDim,
            kUbZHalf);
        set_flag(PIPE_MTE2, PIPE_V, EVENT_ID1);
      }
      ...
      if constexpr (!UseQkGroupCache) {
        ...
        mega_gdn_decode_pto::VectorBarrier();
        // l2norm_fwd returns BF16 Q/K to the framework recurrent operator.
        // Preserve that boundary before applying the FP32 query scale.
        TCVT(q_half, q_fp32, RoundMode::CAST_RINT);
        mega_gdn_decode_pto::VectorBarrier();
        TCVT(q_fp32, q_half, RoundMode::CAST_NONE);
        mega_gdn_decode_pto::VectorBarrier();
        TMULS(q_fp32, q_fp32, 0.0883883461356163f);
        mega_gdn_decode_pto::VectorBarrier();
        mega_gdn_decode_pto::NormalizeQk128<false>(
            k_fp32, norm_square, norm_value, reduce_tmp, scalar_tmp);
        mega_gdn_decode_pto::VectorBarrier();
        TCVT(k_half, k_fp32, RoundMode::CAST_RINT);
        mega_gdn_decode_pto::VectorBarrier();
        TCVT(k_fp32, k_half, RoundMode::CAST_NONE);
        mega_gdn_decode_pto::VectorBarrier();
      }
      ...
      WriteCheckpoint(ssm_state_out_handle,
                      read_state_indices_handle[batch_idx * sequence_length + token_idx],
                      1,
                      ssm_checkpoint_stride,
                      head_idx,
                      0);
