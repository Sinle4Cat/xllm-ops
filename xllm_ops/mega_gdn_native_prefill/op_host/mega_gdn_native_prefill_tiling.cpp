...
#include "mega_gdn_native_prefill_tiling.h"
...
static ge::graphStatus TilingFunc(gert::TilingContext *context)
{
    ...
    const auto* native_attrs = context->GetAttrs();
    if (!native_attrs) return ge::GRAPH_FAILED;
    const auto* conv_stride = native_attrs->GetAttrPointer<int64_t>(2);
    const auto* ssm_stride = native_attrs->GetAttrPointer<int64_t>(3);
    const auto* conv_rows = native_attrs->GetAttrPointer<int64_t>(4);
    const auto* slots = native_attrs->GetAttrPointer<int64_t>(5);
    if (!conv_stride || !ssm_stride || !conv_rows || !slots ||
        *conv_rows < 3 || *conv_rows > 19 || *slots < 2 ||
        *slots > std::numeric_limits<int32_t>::max() ||
        *conv_stride < *conv_rows * conv_dim64 ||
        *ssm_stride < heads64 * 128 * 128 ||
        *conv_stride > std::numeric_limits<int32_t>::max() ||
        *ssm_stride > std::numeric_limits<int32_t>::max() ||
        conv_state.GetDimNum() != 1 || ssm_cache.GetDimNum() != 1 ||
        conv_state.GetDim(0) != (*slots - 1) * *conv_stride + *conv_rows * conv_dim64 ||
        ssm_cache.GetDim(0) != (*slots - 1) * *ssm_stride + heads64 * 128 * 128)
        return ge::GRAPH_FAILED;
    const int64_t checkpoint_stride64 = 1;
    ...
    MegaGdnNativePrefillTilingData tiling;
    ...
    tiling.set_conv_state_len(
        uint32_t(3));
    tiling.set_ssm_state_slots(static_cast<uint32_t>(*slots));
    tiling.set_conv_slot_stride(*conv_stride);
    tiling.set_ssm_slot_stride(*ssm_stride);
...
struct MegaGdnNativePrefillCompileInfo {};
...
IMPL_OP_OPTILING(MegaGdnNativePrefill)
    .Tiling(TilingFunc)
    .TilingParse<MegaGdnNativePrefillCompileInfo>(TilingParse);
}  // namespace optiling
