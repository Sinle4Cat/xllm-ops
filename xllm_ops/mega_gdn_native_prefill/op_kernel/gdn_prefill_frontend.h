...
public:
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR weight, GM_ADDR convStateIn,
                                ...
                                const CausalConv1dTilingData *tilingData, int64_t nativeStride)
    {
        this->ResetRuntimeState(tilingData);
        this->nativeReadStride_ = nativeStride;
        this->nativeWriteStride_ = nativeStride;
...
#endif
    {
        ...
        const bool validWrite = writeCacheIndex >= 0 &&
            writeCacheIndex < static_cast<int32_t>(this->tilingData_->numCacheLines);
        if (task.valid && validWrite) {
            this->ProcessFnChunk(
#if defined(GDN_PREFILL_ARCH_A5)
            ...
            // Slot 0 becomes -1 at the native API boundary. ProcessFnChunk
            // always writes its tail, so passing that sentinel would write
            // one physical slot BEFORE the bank during single-row warmup.
            this->LoadWeightAndBias(task.channelStart, task.baseDimSize);
            const int32_t tileLen = task.tokenEnd - task.tokenStart;
            this->InitRingSeqSplit(
                -1, false, 0, task.tokenStart, tileLen,
                task.channelStart, task.baseDimSize,
                static_cast<int32_t>(this->tilingData_->dim));
            this->RunSeq(
                task.tokenStart, tileLen, task.channelStart, task.baseDimSize,
                static_cast<int32_t>(this->tilingData_->dim));
            this->DrainTaskMte3();
        }
        this->ReleaseEvents();
                                   ...
                                   bool useCompactSnapshot)
    {
        ...
        if (task.valid && batch > 0) {
            ...
            while (cursor < task.tokenEnd && seq < batch) {
                ...
                const int32_t readCacheIndex = (readStateIndices[seq] > 0 ? readStateIndices[seq] : -1);
                const int32_t writeCacheIndex = (writeStateIndices[seq] > 0 ? writeStateIndices[seq] : -1);
                if (tileLen > 0 && readCacheIndex <
                    ...
                    // A null cache slot is also used by model warmup. The
                    // downstream GDN stages still consume every token, so
                    // skipping the convolution leaves packed Q/K/V undefined.
                    // Compute a stateless segment and suppress cache writes.
                    this->LoadWeightAndBias(task.channelStart, task.baseDimSize);
                    this->InitRingSeqSplit(
                        -1, false, seqStart, cursor, tileLen,
                        task.channelStart, task.baseDimSize,
                        static_cast<int32_t>(this->tilingData_->dim));
                    this->RunSeq(
                        cursor, tileLen, task.channelStart, task.baseDimSize,
                        static_cast<int32_t>(this->tilingData_->dim));
                    this->DrainTaskMte3();
                }
                cursor = tileEnd;
        ...
        int32_t batch) const
    {
        ...
        for (int32_t readSeq = 0; readSeq < batch; ++readSeq) {
            const int32_t readCacheIndex = (readStateIndices[readSeq] > 0 ? readStateIndices[readSeq] : -1);
            if (readCacheIndex < 0) {
                continue;
            }
            for (int32_t writeSeq = 0; writeSeq < batch; ++writeSeq) {
                if (readCacheIndex == (writeStateIndices[writeSeq] > 0 ? writeStateIndices[writeSeq] : -1)) {
                    return true;
                }
            }
        }
        ...
        int32_t batch)
    {
        ...
        for (int32_t seq = 0; seq < batch; ++seq) {
            ...
            for (int32_t pos = 0; pos < stateLen; ++pos) {
                const int64_t srcOffset =
                    static_cast<int64_t>(readCacheIndex) * this->nativeReadStride_ + pos * dim +
                    task.channelStart;
                const int64_t dstOffset =
    ...
    {
        this->nativeReadStride_ = this->tilingData_->stateLen * this->tilingData_->dim;
...
__aicore__ inline void RunConv(GM_ADDR mixedQkv, GM_ADDR convWeight,
                           ...
                           uint32_t batchSize, uint32_t convStateLen, int64_t nativeStride)
{
    ...
    op.Init(mixedQkv, convWeight, convStateIn, convStateOut, packedQkv, &tiling, nativeStride);
    ...
    if (batchSize == 1) {
#if defined(GDN_PREFILL_ARCH_A5)
        const int32_t readCacheIndex = (readIndices[0] > 0 ? readIndices[0] : -1);
        const int32_t writeCacheIndex = (writeIndices[0] > 0 ? writeIndices[0] : -1);
        ...
        op.ProcessSingle(readCacheIndex, writeCacheIndex,
                         useCompactSnapshot);
#else
        op.ProcessSingle((readIndices[0] > 0 ? readIndices[0] : -1), (writeIndices[0] > 0 ? writeIndices[0] : -1));
#endif
    } else {
        const bool useCompactSnapshot =
