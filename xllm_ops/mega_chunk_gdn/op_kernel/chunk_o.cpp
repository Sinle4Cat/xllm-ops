    ...
    int32_t local_rows)
{
  ...
  constexpr int32_t OutputPublicAddr =
      std::is_same_v<DTYPE_Q, GDN_PUBLIC_DTYPE>
          ? OutputComputeAddr
          : 164608;
  ...
  UbND<DTYPE_Q, HalfChunk, HiddenSize, HalfChunk, HiddenSize,
       PadValue::Zero>
      output_compute;
...
AICORE inline void PublishQKTile(
    __gm__ DTYPE_Q *q_handle, __gm__ DTYPE_Q *k_handle,
    __gm__ DTYPE_Q *qk_mailbox, int64_t core_id,
    int64_t qk_offset, int32_t qk_stride, int32_t valid_rows,
    int32_t qk_ready_flag, uint32_t strong_publish)
{
#if defined(__DAV_C220_CUBE__)
  L1Mat<DTYPE_Q, ChunkSize, HiddenSize> q_l1;
  TASSIGN(q_l1, 0);
  L1Mat<DTYPE_Q, ChunkSize, HiddenSize> k_l1;
  TASSIGN(k_l1, 32768);
  ...
  {
    L1Mat<DTYPE_Q, ChunkSize, HiddenSize, DYNAMIC, DYNAMIC>
        q_load(valid_rows, HiddenSize);
    TASSIGN(q_load, 0);
    GmShape2D shape(valid_rows, HiddenSize);
    GmStride2D stride(qk_stride);
    GmTensor2D<DTYPE_Q> q_global(q_handle + qk_offset, shape,
                                 stride);
  ...
  }
  {
    ...
    GmShape2D shape(valid_rows, HiddenSize);
    GmStride2D stride(qk_stride);
    GmTensor2D<DTYPE_Q> k_global(k_handle + qk_offset, shape,
                                 stride);
  ...
  {
    TileLeft<DTYPE_Q, ChunkSize, HiddenSize,
             ChunkSize, HiddenSize> l0a;
    TileRight<DTYPE_Q, HiddenSize, ChunkSize,
    ...
    L1MatZN<DTYPE_Q, HiddenSize, ChunkSize> k_zn;
    TRESHAPE(k_zn, k_l1);
...

  {
    ...
    GlobalTensor<DTYPE_Q, decltype(shape),
...
AICORE inline void PublishQKVTile(
    __gm__ DTYPE_Q *v_handle,
    __gm__ DTYPE_Q *qk_gated_mailbox,
    __gm__ DTYPE_Q *qkv_mailbox, int64_t core_id,
    ...
    int32_t qkv_bottom_tail_ready_flag = -1)
{
  ...
  L1Mat<DTYPE_Q, HalfChunk, ChunkSize> qk_gated_l1;
  ...
  TASSIGN(qkv_l0, 0);
  TileRight<DTYPE_Q, ChunkSize, HiddenSize,
  ...
  // cross-core wait so MTE2 can run while Vec produces QK_gated.
  {
    ...
    GmTensor2D<DTYPE_Q> v_global(v_handle + v_offset, shape,
                                 stride);
  ...
  for (int32_t half_idx = 0; half_idx < 2; ++half_idx) {
    if (half_idx != 0 && split_bottom_quarters) {
      for (int32_t quarter_idx = 0; quarter_idx < 2; ++quarter_idx) {
        ...
        L1Mat<DTYPE_Q, QuarterChunk, ChunkSize> qk_quarter_l1;
        TASSIGN(qk_quarter_l1, 98304);
        if (quarter_valid_rows > 0) {
          L1Mat<DTYPE_Q, QuarterChunk, ChunkSize,
                DYNAMIC, DYNAMIC>
        ...
        TileLeft<DTYPE_Q, QuarterChunk, ChunkSize,
    ...
    if (half_valid_rows > 0) {
      L1Mat<DTYPE_Q, HalfChunk, ChunkSize,
    ...
    TileLeft<DTYPE_Q, HalfChunk, ChunkSize,
    ...
    int32_t qk_gated_bottom_tail_ready_flag = -1)
{
          ...
          qk_global(
              qk_mailbox +
                  ...
                  static_cast<int64_t>(vec_id) * HalfChunk *
                      ChunkSize,
              shape);
      UbND<DTYPE_Q, PrefetchRows, ChunkSize,
  ...
  if (local_rows > 0) {
    if (pipeline_bottom_quarters) {
      UbND<DTYPE_Q, QuarterChunk, ChunkSize,
      ...
      TASSIGN(
          ...
          QKHalfUbAddr +
              QuarterChunk * ChunkSize *
                  static_cast<int32_t>(sizeof(DTYPE_Q)));
      {
        Shape<1, 1, 1, DYNAMIC, DYNAMIC> shape;
        shape.shape[3] = QuarterChunk;
        shape.shape[4] = ChunkSize;
      ...
      UbND<DTYPE_Q, PrefetchRows, ChunkSize,
          ...
          qk_prefetch;
      TASSIGN(qk_prefetch, QKPrefetchUbAddr);
      UbND<DTYPE_Q, RemainingRows, ChunkSize,
      ...
      wait_flag(PIPE_V, PIPE_MTE3, EVENT_ID0);
      {
        ...
        GlobalTensor<DTYPE_Q, decltype(store_shape),
                     Stride<1, 1, 1, ChunkSize, 1>>
            gated_global(
                qk_gated_mailbox +
                    core_id * static_cast<int64_t>(ChunkSize) *
...
AICORE inline void ConsumeQKVTile(
    __gm__ DTYPE_Q *precomputed_qs_handle,
    __gm__ DTYPE_Q *qkv_mailbox,
    __gm__ GDN_PUBLIC_DTYPE *output_handle,
    __gm__ GDN_PUBLIC_DTYPE *z_handle,
    ...
    int32_t qkv_bottom_tail_ready_flag = -1)
{
  ...
  if (local_rows > 0) {
    ...
    UbND<DTYPE_Q, HalfChunk, HiddenSize,
            ...
            qkv_global(
                qkv_mailbox +
                    ...
                    static_cast<int64_t>(vec_id) * HalfChunk *
                        HiddenSize,
                shape);
        UbND<DTYPE_Q, QuarterChunk, HiddenSize,
...
AICORE void GDN_CHUNK_O_KERNEL(
    __gm__ DTYPE_Q *Q_handle, __gm__ DTYPE_Q *K_handle, __gm__ DTYPE_Q *V_handle,
    __gm__ DTYPE_Q *S_handle, __gm__ float *G_handle,
    __gm__ float *Msk_handle,
    __gm__ DTYPE_Q *workspace_qk_handle,
    __gm__ DTYPE_Q *workspace_qs_qkv_handle,
    __gm__ DTYPE_Q *workspace_qk_gated_handle,
    __gm__ DTYPE_Q *workspace_ping_qk_handle,
    __gm__ DTYPE_Q *workspace_ping_qs_qkv_handle,
    __gm__ DTYPE_Q *workspace_ping_qk_gated_handle,
    ...
    __gm__ GDN_PUBLIC_DTYPE *norm_weight_handle)
{
  ...
  TileAcc<float, ChunkSize, ChunkSize,
          ChunkSize, ChunkSize> qk_l0;
  TASSIGN(qk_l0, 0);
  L1Mat<DTYPE_Q, HiddenSize, HiddenSize> s_l1;
  TASSIGN(s_l1, 65536);
  ...
  //   qk_ub:      QK scores in float [C/2, C]         @ 33280   (after cast from DTYPE_Q)
  //   g_v_ub:     this sub-block's gate slice [1, C/2] @ 66048
  //   coeff_ub:   gating coefficients [C/2, C] float  @ 66304
  //   qk_ub_half: QK in DTYPE_Q [C/2, C]                @ 99072
  //   qs_ub_half: QS in DTYPE_Q [C/2, D]                @ 115456
  //   qs_ub:      QS in float [C/2, D]               @ 131840
  //   o_ub_half:  output O in DTYPE_Q [C/2, D]           @ 164608
  ...
  UbND<DTYPE_Q, HalfChunk, ChunkSize, HalfChunk, ChunkSize, PadValue::Zero> qk_ub_half;
  TASSIGN(qk_ub_half, QKHalfUbAddr);
  UbND<DTYPE_Q, HalfChunk, HiddenSize, HalfChunk, HiddenSize, PadValue::Zero> qs_ub_half;
  TASSIGN(qs_ub_half, QSHalfUbAddr);
  UbND<float, HalfChunk, HiddenSize> qs_ub;
  TASSIGN(qs_ub, QSUbAddr);
  UbND<DTYPE_Q, HalfChunk, HiddenSize, HalfChunk, HiddenSize, PadValue::Zero> o_ub_half;
  ...
  if (cu_seqlens == nullptr) {
    ...
    for (int64_t work_idx = static_cast<int64_t>(cid);
      ...
      // downstream GEMMs see a clean C×D matrix.
      {
        ...
        GmTensor2D<DTYPE_Q> _gm(Q_handle + qk_off, _gs, _stride);
        TLOAD(_l1, _gm);
      ...
      // ── Load K [valid_rows × D] from GM → L1 ────────────────────────
      {
        ...
        GmShape2D _gs(valid_rows, HiddenSize);
        GmStride2D _stride(BSND_QK_STRIDE);
        GmTensor2D<DTYPE_Q> _gm(K_handle + qk_off, _gs, _stride);
        TLOAD(_l1, _gm);
        if (valid_rows != ChunkSize) TFILLPAD(_l1, _l1);
      }
      ...
      // effectively transposing K before TEXTRACT loads it into L0B.
      {
        ...
        L1MatZN<DTYPE_Q, HiddenSize, ChunkSize> _bzn; TRESHAPE(_bzn, k_l1); TEXTRACT(_l0b, _bzn, 0, 0);
      ...
      {
        L1Mat<DTYPE_Q, HiddenSize, HiddenSize, DYNAMIC, DYNAMIC> _l1(HiddenSize, HiddenSize);
        TASSIGN(_l1, 65536);
      ...
      {
        TileLeft<DTYPE_Q, ChunkSize, HiddenSize, ChunkSize, HiddenSize> _l0a;
        TileRight<DTYPE_Q, HiddenSize, HiddenSize, HiddenSize, HiddenSize> _l0b;
        TASSIGN(_l0a, 0x0); TASSIGN(_l0b, 0x0);
        auto _we = EVENT_ID1;
        set_flag(PIPE_MTE2, PIPE_MTE1, _we); wait_flag(PIPE_MTE2, PIPE_MTE1, _we);
        set_flag(PIPE_M, PIPE_MTE1, _we); wait_flag(PIPE_M, PIPE_MTE1, _we);
      ...
      // ── Store QK [C × C] from L0C → GM workspace (FP32→DTYPE_Q cast) ───
      ...
      {
        L1Mat<DTYPE_Q, ChunkSize, ChunkSize, DYNAMIC, DYNAMIC> _l1(ChunkSize, ChunkSize);
        TASSIGN(_l1, 98304);
        Shape<1, 1, 1, DYNAMIC, DYNAMIC> _gs;
        _gs.shape[3] = ChunkSize; _gs.shape[4] = ChunkSize;
        GlobalTensor<DTYPE_Q, decltype(_gs), pto::Stride<1, 1, 1, ChunkSize, 1>> _gm(
      ...
      // ── Load V [valid_rows × D] from GM → L1 ────────────────────────
      {
        ...
        GmTensor2D<DTYPE_Q> _gm(V_handle + v_off, _gs, _stride);
        TLOAD(_l1, _gm);
        if (valid_rows != ChunkSize) TFILLPAD(_l1, _l1);
      }
      ...
      {
        TileLeft<DTYPE_Q, ChunkSize, ChunkSize, ChunkSize, ChunkSize> _l0a;
        TileRight<DTYPE_Q, ChunkSize, HiddenSize, ChunkSize, HiddenSize> _l0b;
        TASSIGN(_l0a, 0x0); TASSIGN(_l0b, 0x0);
        auto _we = EVENT_ID1;
        set_flag(PIPE_MTE2, PIPE_MTE1, _we); wait_flag(PIPE_MTE2, PIPE_MTE1, _we);
        set_flag(PIPE_M, PIPE_MTE1, _we); wait_flag(PIPE_M, PIPE_MTE1, _we);
    ...
    while (group_work < group_work_count) {
      ...
      __gm__ DTYPE_Q *current_qk_mailbox = workspace_qk_handle;
      ...
      if (next_item < h_o_total_items && reuse_group_qk == 0) {
        ...
        const int64_t next_qk_offset =
            (next_chunk_token_start * static_cast<int64_t>(Hg) +
             next_head_group) *
            static_cast<int64_t>(HiddenSize);
        __gm__ DTYPE_Q *next_qk_mailbox =
            next_slot == 0
                ? workspace_qk_handle
                : workspace_ping_qk_handle +
                      static_cast<int64_t>(block_num) * WsQKSize;
  ...
  if (cu_seqlens == nullptr) {
    ...
    for (int64_t work_idx = static_cast<int64_t>(cid);
      ...
      // ── Load QK [C/2 × C] from workspace → UB ───────────────────────
      {
        ...
        GlobalTensor<DTYPE_Q, decltype(_gs), pto::Stride<1, 1, 1, ChunkSize, 1>> _gm(
            workspace_qk_handle +
                ...
                static_cast<int64_t>(vid) * HalfChunk * ChunkSize, _gs);
        UbND<DTYPE_Q, HalfChunk, ChunkSize, DYNAMIC, DYNAMIC, PadValue::Zero> _ld(local_rows, ChunkSize);
      ...
      // ── Load QS [C/2 × D] from workspace → UB ───────────────────────
      {
        ...
        UbND<DTYPE_Q, HalfChunk, HiddenSize, DYNAMIC, DYNAMIC, PadValue::Zero> _ld(local_rows, HiddenSize);
      ...
      wait_flag(PIPE_V, PIPE_MTE3, EVENT_ID0);
      {
        ...
        GlobalTensor<DTYPE_Q, decltype(_gs), pto::Stride<1, 1, 1, ChunkSize, 1>> _gm(
            workspace_qk_gated_handle +
                ...
                static_cast<int64_t>(vid) * HalfChunk * ChunkSize, _gs);
        UbND<DTYPE_Q, HalfChunk, ChunkSize, DYNAMIC, DYNAMIC> _st(local_rows, ChunkSize);
      ...
      //       = intra_chunk_attention + inter_chunk_state_contribution
      // TCVT DTYPE_Q→float for QKV, then add the state contribution.
    ...
    while (group_work < group_work_count) {
      ...
      for (int32_t group_lane = 0; group_lane < GROUP; ++group_lane) {
        ...
        if (next_lane < GROUP) {
          ...
          __gm__ DTYPE_Q *next_gated_mailbox =
              next_inplace_lane2
                  ? lane2_mailbox
                  : (next_slot == 0
                         ? workspace_qk_gated_handle
        ...
        for (int32_t h = 0; h < H; ++h) {
          ...
          if (consumer_owner == static_cast<int64_t>(cid)) {
            ...
            if (local_rows == 0) {
              ...
              TMUL(qk_ub, qk_ub, coeff_ub);
              TCVT(qk_ub_half, qk_ub, pto::RoundMode::CAST_NONE);  // float→DTYPE_Q for GM store
              ...
              set_flag(PIPE_MTE2, PIPE_V, EVENT_ID0);
              wait_flag(PIPE_MTE2, PIPE_V, EVENT_ID0);
              TCVT(qs_ub, qs_ub_half, pto::RoundMode::CAST_NONE);  // DTYPE_Q→float for Vec math
