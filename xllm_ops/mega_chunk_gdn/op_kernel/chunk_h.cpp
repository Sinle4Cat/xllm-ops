...
AICORE PTO_INLINE void paired_cube_ws(
    __gm__ DTYPE_Q *W_handle, __gm__ DTYPE_Q *workspace_handle,
    ...
    TileAcc<float, C, D, C, D> &ws_l0)
{
  ...
  wait_flag(PIPE_MTE2, PIPE_MTE1, EVENT_ID2);
  {
    ...
    GmTensor2D<DTYPE_Q> s_global(workspace_handle + ws_s_base, s_shape,
                                 s_stride);
    TLOAD(s_l1, s_global);
  }
  ...
  if constexpr (GDN_PAIR_DEBUG_STAGE == 14 ||
    ...
    TileMatL0A<DTYPE_Q, C, D, C, D> w_l0a;
    TileMatL0B<DTYPE_Q, D, D, D, D> s_l0b;
    TASSIGN(w_l0a, 0);
    TASSIGN(s_l0b, 0);
...

  {
    ...
    GmTensor2D<DTYPE_Q> ws_global(workspace_handle + ws_ws_base, ws_shape,
                                  ws_stride);
    ...
    TileAcc<float, C, D, C, D> &ws_l0)
{
  ...
  GmTensor2D<DTYPE_Q> q_global(Q_handle + q_offset, q_shape, q_stride);
  DynMatL1<DTYPE_Q, C, D> q_l1_load(valid, D);
  TASSIGN(q_l1_load, D * D * static_cast<int32_t>(sizeof(DTYPE_Q)));
  ...
  GmTensor2D<DTYPE_Q> qs_global(S_handle + qs_offset, qs_shape, qs_stride);
...
AICORE PTO_INLINE void paired_cube_kv(
    __gm__ DTYPE_Q *V_handle, __gm__ DTYPE_Q *workspace_handle,
    ...
    TileAcc<float, D, D, D, D> &kv_l0)
{
  ...
  }
  {
    ...
    GmTensor2D<DTYPE_Q> k_global(workspace_handle + ws_k_base, k_shape,
                                 k_stride);
    DynMatL1<DTYPE_Q, D, C> k_l1_load(D, C);
    TASSIGN(k_l1_load,
            (D * D + C * D) * static_cast<int32_t>(sizeof(DTYPE_Q)));
  ...
  }
  {
    ...
    GmTensor2D<DTYPE_Q> v_global(V_handle + v_offset, v_shape,
                                 v_gm_stride);
    DynMatL1<DTYPE_Q, C, D> v_l1_load(valid, D);
    TASSIGN(v_l1_load,
            (D * D + C * D + D * C) *
      ...
      k_l1, v_l1, kv_l0, true);
  {
    ...
    GmTensor2D<DTYPE_Q> kv_global(workspace_handle + ws_kv_base, kv_shape,
                                  kv_stride);
...
AICORE PTO_INLINE void paired_vec_init_zero(
    __gm__ DTYPE_Q *S_handle, __gm__ DTYPE_Q *workspace_handle,
    int64_t head, int64_t chunk_offset, int32_t H, int32_t vid,
    int64_t ws_s_base,
    TileUbDataND<float, C / 2, D, C / 2, D> &state_ub,
    TileUbDataND<DTYPE_Q, C / 2, D, C / 2, D> &state_half)
{
...
AICORE PTO_INLINE void paired_vec_front(
    __gm__ DTYPE_Q *K_handle, __gm__ DTYPE_Q *U_handle,
    __gm__ float *G_handle, __gm__ DTYPE_Q *V_handle,
    __gm__ DTYPE_Q *workspace_handle, int64_t head, int64_t head_g,
    ...
    int64_t ws_k_base,
    TileUbDataND<float, C / 2, D, C / 2, D> &state_ub,
    TileUbDataND<DTYPE_Q, C / 2, D, C / 2, D,
...
AICORE PTO_INLINE void paired_vec_store_final(
    __gm__ DTYPE_Q *FS_handle, __gm__ float *final_state_cache,
    ...
    TileUbDataND<DTYPE_Q, C / 2, D, C / 2, D> &state_half)
{
  ...
  if constexpr (StoreFinalStateCache) {
    ...
    GmTensor2D<DTYPE_Q> fs_global(FS_handle + fs_offset, fs_shape,
                                  fs_stride);
    ...
    int64_t state_cache_slots)
{
  ...
  static_assert(GDN_H_WORKSPACE_PAD_BYTES % sizeof(DTYPE_Q) == 0);
  constexpr int32_t WS_FIELD_STRIDE =
      DD + GDN_H_WORKSPACE_PAD_BYTES / sizeof(DTYPE_Q);
  ...
  TileMatL1<DTYPE_Q, C, D, C, D> q_l1;
  TASSIGN(q_l1, D * D * sizeof(DTYPE_Q));
  TileAcc<float, C, D, C, D> ws_l0;
  TASSIGN(ws_l0, 0);
  TileMatL1<DTYPE_Q, D, C, D, C> k_l1;
  ...
  constexpr int32_t S_ALT_UB =
      S_UB_HALF + HalfC * D * sizeof(DTYPE_Q);
  ...
  if (use_paired_head_pipeline && cid < paired_core_count) {
    ...
    if constexpr (GDN_PAIR_DEBUG_STAGE == 12) {
      ...
      GmStride2D w_stride(H * D);
      GmTensor2D<DTYPE_Q> w_global(W_handle + w_offset, w_shape, w_stride);
      DynMatL1<DTYPE_Q, C, D> w_l1_load(C, D);
    ...
    if constexpr (GDN_PAIR_DEBUG_STAGE == 13) {
      ...
      GmShape2D s_shape(D, D);
      GmStride2D s_stride(D);
      GmTensor2D<DTYPE_Q> s_global(workspace_handle + ws_s_base0, s_shape,
                                   s_stride);
  ...
  for (int64_t wi = 0; wi < (total_work + block_num - 1) / block_num; ++wi) {
    ...
    for (int32_t ci = 0; ci < num_chunks; ++ci) {
      ...
      wait_flag(PIPE_S, PIPE_MTE2, EVENT_ID0);
      {
        ...
        GmShape2D s_shape(D, D);
        GmStride2D s_stride(D);
        GmTensor2D<DTYPE_Q> s_global(S_handle + s_state_offset, s_shape,
                                     s_stride);
        DynMatL1<DTYPE_Q, D, D> s_l1_load(D, D);
        TASSIGN(s_l1_load, 0);
      ...
      if (emit_precomputed_qs) {
        ...
        GmTensor2D<DTYPE_Q> q_global(Q_handle + q_offset, q_shape, q_stride);
        DynMatL1<DTYPE_Q, C, D> q_l1_load(static_cast<int32_t>(valid), D);
        TASSIGN(q_l1_load,
                D * D * static_cast<int32_t>(sizeof(DTYPE_Q)));
        TLOAD(q_l1_load, q_global);
        if (valid != C) {
          TFILLPAD(q_l1_load, q_l1_load);
        }
...

      {
        GmShape2D ws_shape(C, D);
        GmStride2D ws_stride(D);
        GmTensor2D<DTYPE_Q> ws_global(workspace_handle + ws_ws_base,
                                   ws_shape, ws_stride);
        DynAccTile<float, C, D> ws_store(C, D);
        TASSIGN(ws_store, 0);
      ...
      if (emit_precomputed_qs) {
        ...
        GmShape2D qs_shape(C, D);
        GmStride2D qs_stride(D);
        GmTensor2D<DTYPE_Q> qs_global(S_handle + qs_offset, qs_shape,
                                     qs_stride);
        DynAccTile<float, C, D> qs_store(C, D);
        TASSIGN(qs_store, 0);
        TSTORE(qs_global, qs_store);
...

      {
        GmShape2D kv_shape(D, D);
        GmStride2D kv_stride(D);
        GmTensor2D<DTYPE_Q> kv_global(workspace_handle + ws_kv_base,
                                   kv_shape, kv_stride);
        DynAccTile<float, D, D> kv_store(D, D);
  ...
  for (int64_t wi = 0; wi < (total_work + block_num - 1) / block_num; ++wi) {
    ...
    if (has_initial_state != 0) {
      if constexpr (LoadInitialStateCache) {
        ...
        GmTensor2D<DTYPE_Q> h0_global(H0_handle + h0_offset, h0_shape,
                                     h0_stride);
