...
AICORE inline void mega_solve_tril(__gm__ DTYPE_Q *out, __gm__ DTYPE_Q *in, __gm__ DTYPE_Q *minus_id, uint32_t matrix_size,
                                   ...
                                   uint32_t is_lower,
                                   __gm__ DTYPE_Q *packed_workspace,
                                   bool use_precomputed_m_neg = false)
{
    ...
    mk_solve::runKernelTriInvRecUnroll<DTYPE_Q, float, GDN_C, 1, true,
                                       DTYPE_Q>(
    ...
    if constexpr (WaitForKktReady) {
        mk_solve::runKernelTriInvRecUnroll<DTYPE_Q, float, GDN_C,
                                           ReadyGroupSize,
                                           true, DTYPE_Q, true,
    ...
    mk_solve::runKernelTriInvRecUnroll<
        DTYPE_Q, float, GDN_C, MEGA_CHUNK_GDN_SOLVE_TILES_PER_ITER, true,
        DTYPE_Q, false, PrecomputedAuxiliary>(out, in, minus_id,
    ...
    if (num_matrices <= get_block_num())
        ...
        mk_solve::runKernelTriInvRecUnroll<DTYPE_Q, float, GDN_C, 2, true,
                                           DTYPE_Q, false,
                                           PrecomputedAuxiliary>(
            out, in, minus_id, num_matrices, num_bsnd_heads, cu_seqlens,
            is_lower, packed_workspace, use_precomputed_m_neg);
    else
        mk_solve::runKernelTriInvRecUnroll<DTYPE_Q, float, GDN_C, 4, true,
                                           DTYPE_Q, false,
                                           PrecomputedAuxiliary>(
            out, in, minus_id, num_matrices, num_bsnd_heads, cu_seqlens,
            is_lower, packed_workspace, use_precomputed_m_neg);
...
#define GDN_STAGE_SYNC() SyncAllImpl<false>()
    ...
    if (reuse_group_kk) {
        mk_kkt::BuildGroupKkCache<D, C>(
            reinterpret_cast<__gm__ DTYPE_Q *>(k_ptr),
            reinterpret_cast<__gm__ DTYPE_Q *>(A_inv_ptr),
            reinterpret_cast<__gm__ int32_t *>(cu_seqlens_ptr),
    ...
    if (use_blocked_solve) {
        mk_solve::runKernelTriInvBlocked64ResidentInplaceBSND<DTYPE_Q>(
            reinterpret_cast<__gm__ DTYPE_Q *>(A_ptr),
            reinterpret_cast<__gm__ DTYPE_Q *>(minus_id_ptr), num_matrices,
            static_cast<uint32_t>(H),
            reinterpret_cast<__gm__ int32_t *>(cu_seqlens_ptr));
        wy_a_input_ptr = reinterpret_cast<__gm__ DTYPE_Q *>(A_ptr);
    } else {
#endif
            ...
            if (use_kkt_solve_pipeline) {
                ...
                if (group_size == 2) {
                    mega_solve_tril<true, 2>(
                        reinterpret_cast<__gm__ DTYPE_Q *>(A_inv_ptr),
                        reinterpret_cast<__gm__ DTYPE_Q *>(A_ptr),
                        reinterpret_cast<__gm__ DTYPE_Q *>(minus_id_ptr), C,
                        num_matrices, H,
                        reinterpret_cast<__gm__ int32_t *>(cu_seqlens_ptr),
                        1, reinterpret_cast<__gm__ DTYPE_Q *>(kkt_ws_ptr),
                        use_precomputed_m_neg);
                } else {
    ...
    mk_wy::GDN_WY_FAST_CALL<D, C>(
        ...
        wy_a_input_ptr, reinterpret_cast<__gm__ DTYPE_Q *>(wy_ws_a1_ptr),
    ...
    mk_h::chunk_h_kernel<D, C, StoreFinalStateCache,
        ...
        reinterpret_cast<__gm__ DTYPE_Q *>(q_ptr),
        reinterpret_cast<__gm__ DTYPE_Q *>(k_ptr),
        reinterpret_cast<__gm__ DTYPE_Q *>(w_ptr),
        reinterpret_cast<__gm__ DTYPE_Q *>(u_ptr),
        reinterpret_cast<__gm__ float *>(g_t_ptr),
        reinterpret_cast<__gm__ DTYPE_Q *>(s_ptr),
        reinterpret_cast<__gm__ DTYPE_Q *>(v_new_ptr),
        reinterpret_cast<__gm__ DTYPE_Q *>(fs_ptr),
        reinterpret_cast<__gm__ DTYPE_Q *>(h0_ptr), has_initial_state, 1,
        reinterpret_cast<__gm__ DTYPE_Q *>(h_ws_ptr),
    ...
    mk_o::GDN_CHUNK_O_CALL<D, C, FuseGatedRmsNorm>(
        ...
        reinterpret_cast<__gm__ DTYPE_Q *>(o_ws_gated_ptr),
        reinterpret_cast<__gm__ DTYPE_Q *>(kkt_ws_ptr),
        reinterpret_cast<__gm__ DTYPE_Q *>(wy_ws_a1_ptr),
        reinterpret_cast<__gm__ DTYPE_Q *>(wy_ws_a2_ptr),
