...
namespace ops {
class MegaKdaPrefill : public OpDef {
public:
    explicit MegaKdaPrefill(const char* name) : OpDef(name)
    {
        ...
        Input("beta").ParamType(REQUIRED).DataType({ge::DT_FLOAT}).FormatList({ge::FORMAT_ND});
        ...
        Attr("ffts_addr").AttrType(REQUIRED).Int();
        AICore().AddConfig("ascend910b");
    }
};
OP_ADD(MegaKdaPrefill);
} // namespace ops
