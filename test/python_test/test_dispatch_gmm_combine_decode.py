...
# -*- coding: utf-8 -*-
"""
pytest test for dispatch_gmm_combine_decode (multi-card MoE dispatch+GMM+combine decode).
Migrated from vllm-ascend multicard_ops_a3.

Compares SmallOps (torch_npu native ops chain) vs FusionOp (single fused op).

Dependencies:
  - custom_ops_lib (pybind11 module built from RegisterOps.cpp)
  - torch_npu native MoE ops (npu_moe_distribute_dispatch_v2, npu_grouped_matmul, etc.)
  - HCCL multi-card (ep_world_size=2)

NOTE: This test requires:
  1. custom_ops_lib.so compiled with aclnnDispatchGmmCombineDecode support
  2. At least 2 NPU cards available
  3. HCCL communication environment
"""
import gc
import os
import sys
...
class FusionOp(DecodeMoeOps):
    ...
    def _apply_ops(self, x, expert_ids, smooth_scales, expert_scales, x_active_mask):
        ...
        output, expert_token_nums = custom_ops_lib.dispatch_gmm_combine_decode(
            x,
            expert_ids,
            self.gmm1_weight,
            ...
            0,  # quant_mode
            self.batch_size * self.ep_world_size,  # global_bs
        )
...
def run_once(
    ...
    global_rank_id = local_rank_id  # single node
    device_id = local_rank_id % 8
    ...
    small_ops = SmallOps(*weight_datas, ep_hcomm_info_small, *parameter, dynamic_eplb, w8a8_dynamic, is_nz).npu()
    fused_ops = FusionOp(*weight_datas, ep_hcomm_info_fused, *parameter, dynamic_eplb, w8a8_dynamic, is_nz).npu()
...
def test_dispatch_gmm_combine_decode_base():
    ...
    custom_kwargs["ep_world_size"] = 2
    custom_kwargs["moe_expert_num"] = 8
    custom_kwargs["top_k"] = 4
    custom_kwargs["w8a8_dynamic"] = True
